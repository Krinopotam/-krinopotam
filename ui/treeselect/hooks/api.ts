...
const useAddNodes = (api: ITreeSelectApi) => {
    return useCallback(
        (parentNode: ITreeSelectNode | undefined, newNodes: ITreeSelectNode | ITreeSelectNode[]) => {
            const _newNodes = HelpersObjects.isArray(newNodes) ? (newNodes as ITreeSelectNode[]) : [newNodes as ITreeSelectNode];
            const treeProps = api.getProps();
            const keyField = treeProps.fieldNames?.value || 'id';
            ...
            for (const _newNode of _newNodes) {
                if (!_newNode[keyField]) _newNode[keyField] = HelpersStrings.getUuid();
            }
...
const useUpdateNodes = (api: ITreeSelectApi) => {
    return useCallback(
        (updatedNodes: ITreeSelectNode | ITreeSelectNode[]) => {
            const _updatedNodes = HelpersObjects.isArray(updatedNodes) ? (updatedNodes as ITreeSelectNode[]) : [updatedNodes as ITreeSelectNode];
            const treeProps = api.getProps();
            const keyField = treeProps.fieldNames?.value || 'id';
            const childrenField = treeProps.fieldNames?.children || 'children';
...
const useDeleteNodes = (api: ITreeSelectApi) => {
    return useCallback(
        (removeNodes: ITreeSelectNode | ITreeSelectNode[]) => {
            const _removeNodes = HelpersObjects.isArray(removeNodes) ? (removeNodes as ITreeSelectNode[]) : [removeNodes as ITreeSelectNode];
            const treeProps = api.getProps();
            const keyField = treeProps.fieldNames?.value || 'id';
            const childrenField = treeProps.fieldNames?.children || 'children';
